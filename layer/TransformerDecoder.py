...
class TransformerDecoder (nn.Module):
    ...
    def __init__(self, n_layer: int,
        d_model: int, d_model_ctx: int, d_ff: int, n_head: int) -> None:
        super().__init__()
        self.layers = nn.ModuleList([
            TransformerDecoderLayer(d_model, d_model, d_model_ctx, d_ff, d_model, n_head)
    ...
    self_mask, context_mask: (*, n, n) of Boolean
    output: (*, n, d_model)
    """
...
if __name__=="__main__":
    dec = TransformerDecoder(1,512,512,2048,8)
    x = torch.randn(3, 128, 512)
    ctx = torch.randn(3, 128, 512)
    y = dec(x, ctx, None, None)
    assert(y.shape == x.shape)

    dec6 = TransformerDecoder(6,512,512,2048,8)
    x = torch.randn(3, 256, 512)
    ctx = torch.randn(3, 256, 512)
    self_mask = torch.tril(torch.ones(256,256)).bool()
