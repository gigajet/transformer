...
class Proposal5EncoderLayer(nn.TransformerEncoderLayer):
    r""""
    PROPOSAL 5
    This is the nn.TransformerEncoderLayer, but for each encoder layer
    The FuzzyLayer substitutes the MultiHeadAttention 
    ...
    def __init__(self, d_model, nhead, dim_fuzzy=512, dim_feedforward=2048, dropout=0.1, activation=F.relu,
                 layer_norm_eps=1e-5, batch_first=False, norm_first=False,
                 device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super(Proposal5EncoderLayer, self).__init__(
            d_model=d_model, 
            nhead=nhead, 
...
@register_model('proposal5')
class Proposal5Transformer(FairseqEncoderDecoderModel):
    @staticmethod
    def add_args(parser):
    ...
    def build_model(cls, args, task):
        ...
        model = Proposal5Transformer(encoder, decoder)
        return model

@register_model_architecture('proposal5', 'proposal5_default')
