...
class Proposal4Encoder (FairseqEncoder):
    def __init__(self, max_src_len: int, dictionary, num_layer: int,
        ...
        encoder_layer = Proposal4EncoderLayer(d_model=dim_model,
            nhead=num_head, 
            dim_fuzzy=dim_fuzzy,
            dim_feedforward=dim_feedforward,
            dropout=dropout,
...
@register_model('proposal4')
class Proposal4Transformer(FairseqEncoderDecoderModel):
    @staticmethod
    def add_args(parser):
    ...
    def build_model(cls, args, task):
        ...
        model = Proposal4Transformer(encoder, decoder)
        return model

@register_model_architecture('proposal4', 'proposal4_default')
def mytransformer_default(args):
    # We use ``getattr()`` to prioritize arguments that are explicitly given
    # on the command-line, so that the defaults defined below are only used
    # when no other value has been specified.
    args.num_layer = getattr(args, 'num_layer', 6)
    args.dim_fuzzy = getattr(args, 'dim_fuzzy', 512)
