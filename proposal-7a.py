...
"""
PROPOSAL 7A
FuzzyBlock (with AddNorm) is inserted after Embedding and PositionalEncoding, before Encoder
"""

class Proposal7AEncoder (FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        x = self.nn_encoder(x, mask, src_key_padding)
    ...
    def _fuzzy_block(self, x):
        return self.fuzzyrule_layer(self.membership_layer(x))

@register_model('proposal7A')
class Proposal7ATransformer(FairseqEncoderDecoderModel):
    @staticmethod
    def add_args(parser):
    ...
    def build_model(cls, args, task):
        ...
        encoder = Proposal7AEncoder(args.max_src_len,
        ...
        model = Proposal7ATransformer(encoder, decoder)
        return model

@register_model_architecture('proposal7A', 'proposal7A_default')
